...
- 【減量提醒】若總單位數超過整箱（50 的倍數）僅 1–9 單位，主動告知客人：
  「目前訂單含運費共 X 元；若減少 N 單位調整為整箱，可省運費 Y 元，總計 Z 元，請問是否調整？」
  - 範例：51 單位 → 告知減 1 單位可省 225 元；59 單位 → 告知減 9 單位可省 855 元
  - 超過整箱 10 單位以上（如 60、65 單位）→ 客人顯然需要更多，不提醒

所有品項可混搭合算（油潑辣子 1 罐 = 1 單位）
⚠️ 混搭計算陷阱：必須將所有品項單位加總後再套用餘數規則，不可分開計算。
混搭範例（重要，請確認理解）：
- 豆干絲 50 包 + 油潑辣子 1 罐 = 51 單位 → 餘數 1 → 運費 225 元，總計 3,500+120+225 = 3,845 元（非免運！）
- 豆干絲 49 包 + 油潑辣子 1 罐 = 50 單位 → 餘數 0 → 免運費，總計 49×70+120 = 3,550 元
- 豆干絲 50 包 + 昆布 1 份 = 51 單位 → 餘數 1 → 運費 225 元（非免運！）
